...
set Libero_project_name_CFG1 PF_Avalanche_MiV_RV32_CFG1_BaseDesign

set project_folder_name_CFG2 MiV_CFG2_BD
set project_dir_CFG2 "./$project_folder_name_CFG2"
set Libero_project_name_CFG2 PF_Avalanche_MiV_RV32_CFG2_BaseDesign

set project_folder_name_CFG3 MiV_CFG3_BD
set project_dir_CFG3 "./$project_folder_name_CFG3"
set Libero_project_name_CFG3 PF_Avalanche_MiV_RV32_CFG3_BaseDesign
...
if {"$config" == "CFG1"} then {
	...
	if {[file exists $project_dir_CFG3] == 1} then {
		...
		create_new_project_label
		new_project -location $project_dir_CFG3 -name $Libero_project_name_CFG3 -project_description {} -block_mode 0 -standalone_peripheral_initialization 0 -instantiate_in_smartdesign 1 -ondemand_build_dh 1 -hdl {VERILOG} -family {PolarFire} -die {MPF300TS} -package {FCG484} -speed {STD} -die_voltage {1.0} -part_range {IND} -adv_options {IO_DEFT_STD:LVCMOS 1.8V} -adv_options {RESTRICTPROBEPINS:1} -adv_options {RESTRICTSPIPINS:0} -adv_options {SYSTEM_CONTROLLER_SUSPEND_MODE:0} -adv_options {TEMPR:IND} -adv_options {VCCI_1.2_VOLTR:IND} -adv_options {VCCI_1.5_VOLTR:IND} -adv_options {VCCI_1.8_VOLTR:IND} -adv_options {VCCI_2.5_VOLTR:IND} -adv_options {VCCI_3.3_VOLTR:IND} -adv_options {VOLTR:IND}
		download_cores_all_cfgs
...
if {"$design_flow_stage" == "SYNTHESIZE"} then {
	...
	if {"$config" == "CFG1"} then {
		export_prog_job \
			-job_file_name {PF_Avalanche_MiV_RV32_CFG1_BaseDesign} \
		...
		export_prog_job \
			-job_file_name {PF_Avalanche_MiV_RV32_CFG2_BaseDesign} \
		...
		export_prog_job \
			-job_file_name {PF_Avalanche_MiV_RV32_CFG3_BaseDesign} \
