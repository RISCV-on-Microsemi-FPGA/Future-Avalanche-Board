...
append Libero_project_name_ahb "_AVMPF300TS_01"
append project_dir_ahb "_AVMPF300TS_01"
append project_folder_name_ahb "_AVMPF300TS_01"
...
append Libero_project_name_axi "_AVMPF300TS_01"
append project_dir_axi "_AVMPF300TS_01"
append project_folder_name_axi "_AVMPF300TS_01"
...
proc create_new_project_label_axi { }\
{
    ...
    puts "-------------------------------------------------------------------------"
    puts "-----------------------CREATING NEW PROJECT------------------------------"
	puts "------------MiV_RV32IMA_L1_AXI_BaseDesign_AVMPF300TS_01------------------"
    puts "-------------------------------------------------------------------------"
	puts "-------------------------------------------------------------------------"
}
...
if {"$target" == "AHB"} then {
	if {[file exists $project_dir_ahb] == 1} then {
		...
		source ./import/components/AHB/import_component_and_constraints_pf_avmpf300ts_01_ahb.tcl
		save_project
		puts "-------------------------------------------------------------------------"
		puts "-------------------------------SUCCESS!----------------------------------"
		puts "-------------------------------------------------------------------------"
	...
	if {[file exists $project_dir_axi] == 1} then {
		...
		source ./import/components/AXI/import_component_and_constraints_pf_avmpf300ts_01_axi.tcl
		save_project
		puts "-------------------------------------------------------------------------"
		puts "-------------------------------SUCCESS!----------------------------------"
		puts "-------------------------------------------------------------------------"
...
if {"$design_flow_stage" == "SYNTHESIZE"} then {
	...
	if {"$target" == "AHB"} then {
		export_prog_job \
			-job_file_name {BaseDesign_AVMPF300TS-AHB-ProgramFile} \
		...
		export_prog_job \
			-job_file_name {BaseDesign_AVMPF300TS-AXI-ProgramFile} \
			-export_dir {./../FlashPro_Express_Projects/Programming_Files} \
			-bitstream_file_type {TRUSTED_FACILITY} \
			-bitstream_file_components {}
		save_project
